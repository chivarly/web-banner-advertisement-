...
function scoreClass(v) {
  if (v === null || v === undefined || isNaN(v) || v === 0) return 'na';
...
function barColor(dim) {
  return { trustworthiness: '#818cf8', purchaseIntent: '#fbbf24', aesthetics: '#34d399' }[dim] || '#818cf8';
}
...
async function doLogin() {
  ...
  if (!username || !password) {
    showLoginErr('Please enter your username and password.');
    return;
  }
...
if (TOKEN) {
  apiFetch('/admin/me')
    .then(data => enterDashboard(data.displayName, data.username))
...
async function loadOverview() {
  ...
  try {
    const [ov, cond] = await Promise.all([
      apiFetch('/analytics/overview'),
      apiFetch('/analytics/by-condition')
    ]);
    ...
    $('stat-grid').innerHTML = `
      ...
      <div class="stat-card c-amber">
        <div class="stat-label">Overall Mean Score</div>
    ...
    // Compute dimension means across all conditions
    const dimTotals = { trustworthiness: [], purchaseIntent: [], aesthetics: [] };
    cond.forEach(c => {
      Object.keys(dimTotals).forEach(d => {
        if (c.dimensionMeans[d]?.mean) dimTotals[d].push(c.dimensionMeans[d].mean);
      });
    });
    ...
    $('cond-grid').innerHTML = data.map(c => {
      ...
      const dimHtml = Object.entries(c.dimensionMeans).map(([key, dm]) => {
        ...
        return `
          <div class="dim-row">
            <div class="dim-info">
              <span class="dim-name">${dm.label}</span>
              <span class="dim-val">${fmt(dm.mean)}</span>
            </div>
            <div class="bar-track">
              <div class="bar-fill" style="width:${pct}%;background:${dm.color}"></div>
            </div>
          </div>`;
      }).join('');
      ...
      return `
        <div class="cond-card">
          <div class="cond-header">
            <span class="cond-badge ${isAn?'analogous':'complementary'}">${c.label}</span>
            <span class="cond-n">n = ${c.n}</span>
          </div>
          <div class="cond-meta">
            <strong>${c.product}</strong> · ${c.colorScheme} (${c.background})
          </div>
...
async function loadComparison() {
  ...
  try {
    const [colorData, productData] = await Promise.all([
      apiFetch('/analytics/by-color'),
      apiFetch('/analytics/by-product')
    ]);

    // Color comparison
    const colorGroups = [colorData.analogous, colorData.complementary];
    const colorColors = ['#34d399', '#fbbf24'];
    $('color-compare').innerHTML = colorGroups.map((grp, i) => {
      const qEntries = Object.entries(grp.questionMeans);
      const bars = qEntries.map(([q, qd]) => {
        const pct = qd.mean ? ((qd.mean-1)/4*100).toFixed(1) : 0;
        return `
          <div class="hbar-item">
            <div class="hbar-label-row">
              <span class="hbar-label">${qd.label}</span>
              <span class="hbar-val">${fmt(qd.mean)}</span>
            </div>
            <div class="hbar-track">
              <div class="hbar-fill" style="width:${pct}%;background:${colorColors[i]}"></div>
            </div>
          </div>`;
      }).join('');
      return `
        <div class="compare-card">
          <div class="compare-title">${grp.label} <span style="color:var(--text3);font-size:0.78rem;font-weight:400">(n=${grp.n})</span></div>
          <div style="margin-bottom:12px;font-family:var(--font-d);font-size:1.5rem;font-weight:800;color:var(--text)">
            ${fmt(grp.overallMean)} <span style="font-size:0.75rem;color:var(--text3);font-family:var(--font-b)">overall mean</span>
          </div>
      ...
      const bars = qEntries.map(([q, qd]) => {
        ...
        return `
          <div class="hbar-item">
            ...
            <div class="hbar-track">
              <div class="hbar-fill" style="width:${pct}%;background:${prodColors[i]}"></div>
            </div>
          </div>`;
      }).join('');
      return `
        <div class="compare-card">
          <div class="compare-title">${grp.product} <span style="color:var(--text3);font-size:0.78rem;font-weight:400">(n=${grp.n})</span></div>
          <div style="margin-bottom:12px;font-family:var(--font-d);font-size:1.5rem;font-weight:800;color:var(--text)">
            ${fmt(grp.overallMean)} <span style="font-size:0.75rem;color:var(--text3);font-family:var(--font-b)">overall mean</span>
          </div>
          <div class="hbar-list">${bars}</div>
  ...
  try {
    ...
    for (let qi = 1; qi <= 9; qi++) {
      ...
      tr.innerHTML = `
        ...
        <td style="font-size:0.8rem;color:var(--text)">${FULL_Q_LABELS[qi-1]}</td>
        ${cells}
        <td><span class="score-chip ${scoreClass(avgV)}" style="font-size:0.85rem">${fmt(avgV)}</span></td>
      `;
    ...
    legend.innerHTML = `
      <td colspan="2" style="padding:14px 16px;color:var(--text3);font-size:0.72rem">
        <span class="score-chip high" style="font-size:0.7rem;margin-right:6px">4.0+</span> High &nbsp;
        <span class="score-chip mid" style="font-size:0.7rem;margin-right:6px">3.0–3.99</span> Mid &nbsp;
        <span class="score-chip low" style="font-size:0.7rem;margin-right:6px">&lt;3.0</span> Low
      </td>
      <td colspan="7"></td>
    `;
...
async function loadResponses(paginate = false) {
  ...
  try {
    ...
    if (data.responses.length === 0) {
      tbody.innerHTML = '<tr><td colspan="14" class="empty-state">No responses found.</td></tr>';
    } else {
      tbody.innerHTML = data.responses.map(r => {
        ...
        const qCells = [1,2,3,4,5,6,7,8,9].map(i => {
          ...
          return `<td><span class="score-chip ${scoreClass(v)}" style="font-size:0.72rem;min-width:32px">${v||'—'}</span></td>`;
        }).join('');
        return `
          <tr>
            ...
            <td style="white-space:nowrap;font-size:0.72rem">${relTime(r.submittedAt)}</td>
          </tr>`;
      }).join('');
    }

    // Pagination
    $('pag-info').textContent = `Showing ${Math.min((respPage-1)*RESP_LIMIT+1, respTotal)}–${Math.min(respPage*RESP_LIMIT, respTotal)} of ${respTotal} responses`;
    $('pag-prev').disabled = respPage <= 1;
    $('pag-next').disabled = respPage >= data.pagination.pages;

  } catch(err) {
    $('resp-tbody').innerHTML = `<tr><td colspan="14" class="empty-state">Error: ${err.message}</td></tr>`;
  } finally {
    hide('resp-loading');
  }
}